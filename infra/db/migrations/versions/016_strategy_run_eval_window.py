...
PR #49 round-4 completion sweep (Codex R4-B), extending the 04-4W /
migration 015 fix (``strategy_trials.eval_start_date``/``eval_end_date``,
docs/plans/04-4W-evaluation-window-threading-scope.md) to the
``strategy_runs`` recording path, which was not updated to match.
``StrategyRun`` (``strategy_registry/models.py``) carries ``data_version``
but no evaluation window; two ``backtest``/``walk_forward`` runs over
different windows, recorded under the same ``strategy_id``/``config_hash``/
``data_version``, are indistinguishable and unreconstructable now that
``docs/plans/04-identity-evaluation-context-design.md`` moved
``backtest.start_date``/``backtest.end_date`` out of ``config_hash`` and
into "evaluation context." This migration adds the two columns that close
that gap for ``strategy_runs``, mirroring migration 015's approach exactly.
