...
|---|---|---|---|---|---|
| 04-3 (hypothesis write-path) | `dev/R2-04-3-hypothesis-writepath` | (PR pending) | **In PR review (2026-08-07)** — hypothesis pre-registration write path + CLI (`strategy_registry/hypothesis.py`), `param_grid_json` immutability enforcement, and the `frozen_at` freeze-on-first-linked-trial side effect in `TrialRecorder` (atomic with the trial insert). Carries the 2026-07-23 handoff doc into its PR. PM review + adversarial (APPROVE-WITH-FIXES, no P0/P1; atomicity + cross-strategy-freeze held under attack) → fixes: atomic conditional-UPDATE for the immutability guard (closes a TOCTOU), fast-fail `param_grid_json` shape/serializability validation, freeze audit-log moved after commit. Full `backtesting/tests/` + `tests/strategy_registry/`: 471 passed. | Sonnet 5 medium: 74K (builder) + 62K (adversarial) + 55K (fix round) | Opus 4.8: (in progress) |
