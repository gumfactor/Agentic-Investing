...
| 02B | `dev/R2-02B-config-failclosed` | [#36](https://github.com/gumfactor/Agentic-Investing/pull/36) | **PR open 2026-07-20, in Codex review.** Contract module + fail-closed validation at all 6 backtest entry points; adversarial round found 2 P0 classification-vs-reads defects (execution.* unread, name consumed as strategy_id fallback), fixed in `92ae166` with a full classification sweep (also caught `version` misclassified + a pre-existing test declaring `perfect` fills while running `transaction_cost`). BUG-075 filed. Full suite 2260/2260 | Sonnet 5 medium: 174K (builder incl. fix round) + 103K (adversarial reviewer) | Fable 5: ~40K |
