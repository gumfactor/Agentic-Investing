...
| BUG-083 | Data/Storage (Alembic) | P2 | F2 | Open | Migrations `011_corporate_action_availability.py` and `012_research_identity.py` call `op.alter_column(..., nullable=False, server_default=...)` unwrapped (no dialect guard, no `batch_alter_table`), which breaks with `OperationalError: near "ALTER": syntax error` on SQLite (reproduced during 04-4W's migration-015 F2 fix, which hit and fixed the identical pattern in its OWN new migration rather than 011/012). Additionally, `tests/strategy_registry/test_eval_window_migrations.py` and the equivalent 014-era migration tests never actually execute `upgrade()`/`downgrade()` against a live connection -- they only assert revision-string/callable shape and separately exercise the ORM models via `Base.metadata.create_all()`, which bypasses Alembic entirely and would not have caught this class of unwrapped-`alter_column` SQLite incompatibility. Deliberately not fixed here (04-4W's PM review scoped this to a filed finding, not a fix, to avoid scope creep into unrelated 03A-era migrations). **Recommended pattern updated (PR #50 R1-C, 2026-08-08):** a bare dialect skip (`if dialect.name != "sqlite": op.alter_column(...)`) is NOT sufficient -- SQLite's `ALTER TABLE ... ADD COLUMN ... DEFAULT X` stores that default PERMANENTLY (unlike a droppable Postgres `server_default`), so silently skipping the drop leaves a real persistent default on SQLite. The correct SQLite sequence is: add the column nullable with NO default, backfill via explicit `UPDATE`, then use `op.batch_alter_table(...).alter_column(nullable=False)` (SQLite's `ALTER TABLE` cannot flip `NOT NULL` directly either). See `015_fingerprint_algo_version.py::upgrade()` for the corrected reference implementation, verified end-to-end against a live SQLite connection (backfill correctness, NOT NULL enforcement with no persistent default, and upgrade->downgrade->upgrade round-trip) and against Postgres's offline `as_sql` DDL output. |
| BUG-084 | Backtesting/Promotion (Gate 04, C8 precondition) | P1 | F1 | Fixed (PR #50, 04-4W) | Introduced in 04-4A (merged `a5b7e1c`); found by Codex round-1 review on PR #50. `PromotionPipeline.run()`'s sensitivity gate trusted `ParameterSweeper`'s own `verdict` field without regard to how many DISTINCT variants actually produced a finite result: a single-combination `param_grid` (or a grid where all but one variant failed) trivially satisfies `ParameterSweeper`'s default `min_positive_fraction=0.5` threshold with n=1 data point and skips its std-dispersion gate entirely (guarded by `n_valid > 1`), so `ParameterSweeper` itself reports `verdict="robust"` with zero statistical power -- `overall_passed` could become `True` with no meaningful parameter-sensitivity test having run at all. Since Gate 04's promotion decision is the C8 precondition for any paper-to-live discussion, this let a curve-fit strategy clear the survival funnel undetected. **Fixed in the same PR**: `PromotionPipeline.run()` now recomputes the finite-variant count directly from `sensitivity_result.rows` and force-fails the sensitivity gate (`sensitivity_underpowered`) below a new named constant `MIN_SENSITIVITY_SWEEP_VARIANTS = 3` (`backtesting/validation/promotion_pipeline.py`), regardless of `ParameterSweeper`'s own verdict; the finite-variant count and underpowered flag are recorded in `evidence_json["sensitivity"]` for audit. Three new tests pin the boundary (single-variant, mostly-failed-with-one-survivor, and exactly-at-threshold cases). Not fixed in PR #49 (04-4A) itself per PM scope -- that branch was already merged; the fix lands only in the stacked PR #50. |
| BUG-085 | Backtesting/Promotion (Gate 04, C8 precondition) | P1 | F1 | Fixed (PR #50, 04-4W) | Found by Codex round-2 review on PR #50, immediately after the BUG-084 fix landed in the same PR. `_resolve_frozen_grid` does not reject a `param_grid` list containing repeated values (e.g. `{"portfolio.n_long": [10, 10, 10]}`), so `ParameterSweeper` can produce `MIN_SENSITIVITY_SWEEP_VARIANTS`-many finite rows that all share the SAME `params` -- the BUG-084 fix counted raw finite rows, so this satisfied the new threshold while testing zero actual parameter sensitivity, defeating BUG-084's fix on its first day. **Fixed in the same PR**: the finite-variant count in `PromotionPipeline.run()` (`backtesting/validation/promotion_pipeline.py`) now dedupes rows by their normalized `params` before comparing against `MIN_SENSITIVITY_SWEEP_VARIANTS`. Two new tests pin the duplicate-params-underpowered case and its distinct-params counterpart (`test_duplicate_param_variants_do_not_clear_the_underpowered_gate`, `test_distinct_params_at_min_threshold_still_clears_after_dedupe` in `backtesting/tests/test_promotion_pipeline.py`). **Refined in the same PR (Codex round-3, P2):** the initial dedupe key (`tuple(sorted(row.params.items()))`) raises `TypeError: unhashable type` the moment a candidate value is itself a list/dict (e.g. `{"universe": [{"source": "sp500"}, ...]}`, which `_resolve_frozen_grid`'s non-empty-list-or-tuple check and 04-3's strict-JSON param_grid validation both permit) -- the crash would occur after the walk-forward and sweep had already run, before any promotion decision was persisted. Replaced with `json.dumps(row.params, sort_keys=True)`, which is total over any JSON-serializable value and canonical regardless of key order. New test `test_nested_dict_param_values_do_not_crash_the_dedupe` pins a nested-dict grid deduping correctly without raising. **Refined again in the same PR (Codex round-5, P1 -- the real invariant, not another representation of the same bug):** deduping on ANY representation of `row.params` (tuple, JSON string) was still wrong, because distinct `row.params` dicts can resolve to the IDENTICAL effective backtest config when `param_grid` has overlapping ancestor/descendant dot-paths (e.g. a `"portfolio"` candidate replacing a whole subtree, combined with a `"portfolio.n_long"` candidate that always overrides just that leaf afterward per `_apply_params`'s insertion-order application) -- `_reject_window_override_keys` only blocks paths that are ancestors of `backtest.start_date`/`end_date` specifically, so this combination is otherwise legal. The finite-variant count now dedupes on the FULLY-APPLIED config instead: `_apply_params` (the exact function `ParameterSweeper.sweep` itself uses) is re-run against the exact same dispatched base config (`_config_with_data_version_and_window`, reused rather than reimplemented) for each row, and `json.dumps(..., sort_keys=True, default=str)` of that result is the dedup key -- this can never disagree with what was actually backtested, because it retraces the identical two-step dispatch pipeline using the same two functions. New test `test_ancestor_descendant_grid_paths_collapse_to_same_effective_config` pins 3 rows with 3 distinct raw `params` collapsing to 2 distinct effective configs. |
| BUG-088 | Data/Storage (Strategy Registry) | P2 | F2 | Fixed (PR #50, 04-4W) | Found by Codex round-5 review on PR #50, the same review round as BUG-085's third refinement -- an independent recurrence of BUG-087's exact defect class in a different call site. `StrategyRegistry.record_run` (`strategy_registry/registry.py`) is a public API a caller can reach without going through `EvaluationWindow` (`TrialRecorder`, the only other current caller, always derives `eval_start_date`/`eval_end_date` from an already-validated `EvaluationWindow.start`/`.end`) -- its own `eval_start_date > eval_end_date` check had no type guard, so two `datetime` values (subclasses `date`, passes the comparison) or two ISO strings (compare lexicographically the same way dates do) would be accepted and only fail later, e.g. at the DB `Date`-column write. **Fixed in the same PR**: `EvaluationWindow`'s type check was factored out into a shared `strategy_registry.evaluation_window.require_date(field_name, value)` function (rather than writing a second, possibly-drifting inline copy of the same check), and `record_run` now calls it for both `eval_start_date` and `eval_end_date` before the `>` comparison. Three new tests in `tests/strategy_registry/test_registry.py` pin `datetime` start/end and an ISO string start all raising `TypeError`. **Refined in the same PR (Codex round-6, P2):** the type/order check only ran inside the required-run-type branch, so a caller supplying `eval_start_date`/`eval_end_date` for an OPTIONAL run type (`unit`/`signal_ic`/`paper`/`live`) -- e.g. `record_run(..., run_type="paper", eval_start_date=date(2026, 8, 8), eval_end_date=date(2026, 8, 1))` -- persisted a reversed or incomplete pair with no validation at all, since the nullable column has no DB constraint to catch it either. The type/order check now runs whenever BOTH dates are present regardless of `run_type`, and a new check rejects supplying exactly one of the pair for any `run_type`. Five new tests pin the incomplete-pair (start-only, end-only), reversed, datetime, and both-omitted/both-provided baseline cases for an optional `run_type`. |
| BUG-089 | Backtesting/Promotion (Gate 04, C8 precondition) | P1 | F1 | Fixed (PR #50, 04-4W) | Found by Codex round-6 review on PR #50, the same review round as BUG-088's refinement. The round-5 fix (BUG-085) only corrected the finite-variant COUNT gate (`MIN_SENSITIVITY_SWEEP_VARIANTS`) -- it left `sensitivity_verdict`/`positive_fraction`/`std_oos_sharpe` themselves sourced from `sensitivity_result`, i.e. computed by `ParameterSweeper` over EVERY raw row including duplicate-effective-config ones. A grid with 100 copies of one profitable configuration and 2 distinct losing configurations has 3 distinct effective configs (clears the count gate), but the duplicated winner still dominates `ParameterSweeper`'s own `positive_fraction`/`std` -- 2 of the 3 real configs lose, yet the sweep reports `verdict="robust"`. **Fixed in the same PR**: extracted `ParameterSweeper.sweep`'s verdict-computation arithmetic into a shared `backtesting.validation.parameter_sensitivity.summarize_variants(rows, min_positive_fraction, max_sharpe_std)` function (and `DEFAULT_MIN_POSITIVE_FRACTION`/`DEFAULT_MAX_SHARPE_STD` constants), used both by `ParameterSweeper.sweep` (unchanged behavior, verified via `test_parameter_sensitivity.py`) and by `PromotionPipeline.run()`, which now builds ONE row per distinct effective config (first occurrence) and calls `summarize_variants` on THAT deduped set to get the authoritative `sensitivity_verdict`/mean/std/positive_fraction -- rather than trusting `sensitivity_result`'s raw-row stats. `evidence_json["sensitivity"]` now records both the authoritative (`verdict`/`mean_oos_sharpe`/`std_oos_sharpe`/`positive_fraction`) and raw/uncorrected (`raw_verdict`/`raw_mean_oos_sharpe`/`raw_std_oos_sharpe`/`raw_positive_fraction`) values side by side for audit. New test `test_duplicated_winner_does_not_mask_two_distinct_losers` pins Codex's exact 100-copies-of-a-winner scenario: `raw_verdict == "robust"` (ParameterSweeper's own, uncorrected report) but the authoritative `verdict == "curve_fit"` and `overall_passed is False`. |
| BUG-090 | Data/Storage (MLflow correlation) | P2 | F2 | Fixed (PR #50, 04-4W) | Found by Codex round-6 review on PR #50. `mlflow_logger.py`'s `log_walk_forward_run`/`log_run` (and the identical `_hash_config` in `backtesting/engine/event_loop.py`) set an MLflow `config_hash` tag from a naive full-dict SHA-256 of the DISPATCHED config -- which includes `data_version` and the injected `eval_window` dates. That tag necessarily diverges from the canonical v2 identity hash stored in `StrategyDefinition`/`PromotionDecision.config_hash` (which deliberately EXCLUDES those dates per `docs/plans/04-identity-evaluation-context-design.md`) and changes across two promotion runs of the SAME strategy identity over different windows -- exactly the two runs 04-4W's identity/window split defines as correlatable, defeating that correlation via MLflow. **Fixed in the same PR**: `PromotionPipeline._log_to_mlflow` now passes `tags={"canonical_config_hash": config_hash}` (the authoritative identity hash already in scope) to `log_walk_forward_run`, additive-only -- the existing `config_hash` tag, `_hash_config`, and the `ConfigProvenanceMismatchError` self-consistency check (a different concern: proving the logged config is literally the object the engine ran, not identity correlation) are all left unchanged. New assertion in `test_dsr_fdr_logged_via_mlflow_additive_path` pins the new tag. `event_loop.py`'s duplicate `_hash_config` and `log_run`'s (non-promotion-path) tag were not touched -- out of this fix's scope; no production caller of `log_run` exists yet per its own docstring. |
| BUG-091 | Backtesting/Promotion (Gate 04, C8 precondition) | P1 | F1 | Fixed (PR #50, 04-4W) | Found by Codex round-7 review on PR #50, the same review round as BUG-092 (deferred). "Distinct effective config" (round-5/BUG-085) is still not the same as "distinct BACKTEST BEHAVIOR": a grid entry like `{"description": ["a", "b", "c"]}` produces three genuinely different serialized configs, but `backtesting/config_contract.py` explicitly classifies `"description"` (and other fields, plus the whole `"universe"` section) `INFORMATIONAL` -- accepted by `validate_backtest_config`, but never read by the backtest path. Three rows differing only in an informational field execute the IDENTICAL backtest, so counting them as three distinct variants both wastes compute and, if the base strategy happens to be profitable, can clear `MIN_SENSITIVITY_SWEEP_VARIANTS` and produce a `verdict="robust"` without testing any actual parameter sensitivity. **Fixed in the same PR at TWO layers** (source + defense-in-depth, matching the "fix across the board" standard the operator set after this and the prior six rounds kept finding progressively deeper instances of the same underlying invariant): (1) `PromotionPipeline._resolve_frozen_grid` now rejects (fail-closed, before ANY instrument runs) any `param_grid` key classified `INFORMATIONAL` by `backtesting.config_contract.field_status` -- new tests `test_grid_with_informational_key_fails_closed_before_walk_forward` and `test_grid_with_mixed_informational_and_behavioral_keys_fails_closed` pin this at the source, including the case where an informational key is paired with a genuinely behavioral one. (2) The `n_finite_sensitivity_variants` dedupe computation additionally filters each row's `params` to exclude informational dot-paths before building the effective-config key -- defense in depth for any hypothesis grid FROZEN before this fix shipped (grids are immutable once frozen, so a pre-existing informational-key grid cannot be fixed by the source-level rejection alone). New test `test_informational_grid_key_variants_collapse_to_one_effective_config` pins Codex's exact `"description"` example collapsing 3 rows to 1 variant at the dedupe layer directly. The pre-existing `test_nested_dict_param_values_do_not_crash_the_dedupe` (BUG-085's round-3 unhashable-value fix) used `"universe"` as its example, which this fix now correctly collapses (it always was informational) -- switched to a new, deliberately-unclassified `"custom_metadata"` field so that test's crash-safety property stays independently verifiable from this fix's informational-filtering property. |
| BUG-092 | Data/Storage (Strategy Registry, fingerprint migration) | P1 | F1 | Open (deferred, PR #50, 04-4W) | Found by Codex round-7 review on PR #50. `require_current_fingerprint_algo_version` (`strategy_registry/registry.py`, introduced earlier in this PR's own fingerprint-v2 migration work) raises `FingerprintAlgorithmVersionError` on a version-collision with a pre-v2-hashed row and directs the operator to "re-register this strategy" -- but that remedy has NO working path through the current public API. Traced and confirmed: `add_definition()` computes the new v2 hash, finds no existing `(strategy_id, config_hash)` row under that hash, attempts insert, collides on the `(strategy_id, version)` UNIQUE constraint against the OLD v1-hash row, and the new collision handler re-detects the same algorithm-version mismatch and raises the IDENTICAL `FingerprintAlgorithmVersionError` again -- an infinite retry loop with no exit. `register()` similarly rejects an existing lifecycle row before it could replace one. Any database containing pre-migration-015 definitions therefore has no supported way to bring those strategies back to a usable state without direct manual DB intervention. **Deliberately deferred, not fixed in PR #50** (operator decision, 2026-08-08): this is a different subsystem (fingerprint algorithm versioning / migration workflow) from the evaluation-window and sensitivity-dedupe work this PR is scoped to, predates this session's fixes, and its correct remedy (an explicit rehash migration, or a supported removal-and-re-registration API) is a real design decision deserving its own focused PR rather than folding into an already seven-round review cycle. **Required action:** design and implement either (a) a one-time data migration that recomputes `config_hash`/`fingerprint_algo_version` for every pre-v2 `strategy_definitions` row using the v2 algorithm (the row's identity changes, so downstream FK references and any already-recorded `promotion_decisions`/`strategy_trials`/`strategy_runs` keyed by the OLD hash need an explicit remediation plan too), or (b) a supported `StrategyRegistry` API path (e.g. an explicit `rehash_definition`/`replace_definition` method) that lets an operator move a legacy definition to v2 without hitting the version-collision trap `add_definition()`/`register()` both fall into today. No strategies are currently live (C8 qualification has not started), so this is safe to design properly rather than patch reactively. |
| BUG-093 | Backtesting/Promotion (Gate 04, C8 precondition) | P1 | F1 | Fixed (PR #50, 04-4W) | Found by Codex round-8 review on PR #50, the same defect class as BUG-091 one layer deeper. `backtesting/config_contract.py`'s own docstring defines `CONSUMED` as "read BY KEY -- whether to change behavior... or to individually label a persisted record (`version` -> the `strategy_version` MLflow tag)": a deliberately coarse two-tier classification designed to answer "does the backtest path's declared config claims match what will actually happen/get recorded," NOT "does varying this field change what an individual sweep variant computes." BUG-091's fix (filtering `INFORMATIONAL` keys) therefore missed `CONSUMED` fields that are read only by `BacktestLogger` for post-run record-labeling -- `version` (`strategy_version` MLflow tag) and `reporting.save_positions`/`reporting.save_trades` (gate whether a CSV artifact gets written) -- none of which `BacktestLogger` ever reads per-sweep-variant (it is invoked once, after the sweep, for the aggregate walk-forward leg only). A grid like `{"version": [1, 2, 3]}` therefore still executed the identical backtest 3 times and could clear `MIN_SENSITIVITY_SWEEP_VARIANTS`/produce a false "robust" verdict. **Fixed in the same PR, at the root** (per operator instruction after round 7 to "fix it across the board, no partial patches" -- this is the second time a "reuse an existing classification" shortcut proved insufficiently precise, so the fix targets the actual missing distinction rather than special-casing `"version"` alone): `backtesting/config_contract.py` gains a fourth classification tier, `CONSUMED_LOGGING_ONLY` (fields read by key, but only inside `BacktestLogger`, never inside `BacktestEngine.run`/`WalkForwardValidator.run`/`ParameterSweeper.sweep`/`loader.py`'s actual simulation), and a new canonical predicate `is_behavioral(dot_path) -> bool` (`not in {INFORMATIONAL, CONSUMED_LOGGING_ONLY}`) as the single source of truth for "is this field a meaningful parameter-sensitivity sweep dimension." `version`, `reporting.save_positions`, and `reporting.save_trades` are reclassified `CONSUMED_LOGGING_ONLY` (audited every other `CONSUMED` field against the same question; all others -- `name`, `strategy_id`, `data_version`, `portfolio.n_long`, `backtest.initial_capital`, etc. -- genuinely affect simulation and are unaffected). Both of BUG-091's layers (`_resolve_frozen_grid`'s source-level rejection and the dedupe-level filter) now call `is_behavioral` instead of the narrower `field_status(...) != INFORMATIONAL` check. New tests: `test_is_behavioral_*` in `test_config_contract.py` pin the four-tier classification directly; `test_grid_with_logging_only_key_fails_closed_before_walk_forward` and `test_logging_only_grid_key_variants_collapse_to_one_effective_config` in `test_promotion_pipeline.py` pin Codex's exact `"version"` example at both the source and dedupe layers, mirroring BUG-091's informational-key test pair. |
| BUG-094 | Backtesting/Promotion (Gate 04, C8 precondition) | P1 | F1 | Fixed (PR #50, 04-4W) | Found by Codex round-9 review on PR #50, the same defect class as BUG-091/BUG-093 one further layer deeper -- the same review round as BUG-095 (deferred, different subsystem). `is_behavioral(dot_path)` (BUG-093) correctly classifies individual leaf dot-paths, but a grid key that is a bare enumerated SECTION name (e.g. `"reporting"`, `field_status` status `"section"`) replaces that section's ENTIRE subtree via `_apply_params`/`_set_nested`, and `is_behavioral("reporting")` is True (bare section names are never themselves classified non-behavioral -- only their sub-keys are) even though `"reporting"`'s only two known sub-keys (`save_positions`, `save_trades`) are BOTH `CONSUMED_LOGGING_ONLY` after BUG-093 -- meaning NO possible value assigned to the whole section can ever be behavioral. A grid like `{"reporting": [{"save_trades": True}, {"save_trades": False}, {"save_positions": True}]}` therefore still produced 3 distinct dedupe keys and could clear `MIN_SENSITIVITY_SWEEP_VARIANTS`/produce a false "robust" verdict. **Fixed in the same PR, closing the whole class rather than special-casing `"reporting"`:** `backtesting/config_contract.py` gains `prune_to_behavioral_leaves(config) -> dict`, which recursively strips every non-behavioral leaf from a config at any depth this module's classification reaches (top-level fields, and one level inside each known section) by real leaf classification, not by which grid key introduced the value. `PromotionPipeline.run()`'s dedupe computation (`backtesting/validation/promotion_pipeline.py`) now applies each row's params UNFILTERED first (via `_apply_params`, so ancestor/descendant/section-replace semantics work exactly as the real sweep does), THEN prunes the resulting FULL effective config with `prune_to_behavioral_leaves` before computing the `json.dumps` dedupe key -- replacing the round-7/8 approach of filtering `row.params` before applying it, which could not see non-behavioral leaves nested inside a section-level override. New tests: `test_prune_to_behavioral_leaves_*` in `test_config_contract.py` pin the recursive pruning directly (top-level drop, all-sub-keys-non-behavioral section drop, mixed-section partial-keep); `test_section_level_override_of_all_logging_only_subkeys_collapses` in `test_promotion_pipeline.py` pins Codex's exact `"reporting"` example collapsing 3 rows to 1 variant end-to-end. |
| BUG-095 | Data/Storage (Alembic, SQLite) | P2 | F2 | Open (deferred, PR #50, 04-4W) | Found by Codex round-9 review on PR #50. Migration `015_fingerprint_algo_version.py`'s SQLite branch uses `op.batch_alter_table("strategy_definitions")` (line ~128) to flip the new `fingerprint_algo_version` column `NOT NULL` -- Alembic's batch mode rebuilds the table under SQLite (create new table, copy rows, DROP the old table, rename). If `PRAGMA foreign_keys=ON` (this project's SQLite connections enable it, see `_set_sqlite_pragma`/`_pragma` listeners throughout the test/registry code) and any pre-existing `strategies`/`strategy_runs`/`strategy_trials` row references `strategy_definitions` via FK, SQLite performs an implicit `DELETE FROM` as part of `DROP TABLE` when FK enforcement is on, which fails with `FOREIGN KEY constraint failed` against any referencing child row under the default `RESTRICT`/`NO ACTION` behavior. Because SQLite DDL is non-transactional, a failed run leaves the preceding `ADD COLUMN`/backfill applied, so a retry then hits "column already exists." This only manifests against a SQLite database that already has rows in the referencing child tables at migration time -- the existing migration test suite (`tests/strategy_registry/test_eval_window_migrations.py` and siblings) exercises migrations against a fresh, empty SQLite schema, so it did not catch this. The Postgres branch of this migration (production's actual database per `DATABASE_URL`/TimescaleDB) is unaffected -- Postgres's `ALTER COLUMN ... SET NOT NULL` requires no table rebuild. **Deliberately deferred, not fixed in PR #50** (consistent with how BUG-086/BUG-092 were handled this same review cycle): this is a different subsystem (Alembic/SQLite DDL mechanics) from the sensitivity-dedupe invariant class the rest of round 5-9's fixes closed, is SQLite-only, and requires either disabling/restoring FK enforcement around the specific batch operation or a rebuild sequence that handles dependent tables -- a design decision better made deliberately than folded into an already nine-round review cycle. **Required action:** before this migration is ever run against a non-empty local/dev SQLite database with real `strategies`/`strategy_runs`/`strategy_trials` rows, either (a) wrap the `batch_alter_table` call with `PRAGMA foreign_keys=OFF` / restore-after, or (b) add an explicit test in `test_eval_window_migrations.py` that seeds referencing child rows before running migration 015 upgrade, to catch this class going forward. |
| BUG-096 | Backtesting/Promotion (Gate 04, C8 precondition) | P1 | F1 | Fixed (PR #50, 04-4W) | Found by Codex round-10 review on PR #50, the terminating instance of the defect class BUG-085/091/093/094 kept finding progressively deeper layers of. A grid like `{"portfolio.n_long": [1, 1.0, True]}` produces 3 genuinely distinct serialized values (int/float/bool), but `BacktestEngine.run` applies `int(...)` to `n_long`, so all 3 variants execute with `n_long == 1` -- no config-shape classification (behavioral vs. not, section vs. leaf, informational vs. logging-only) can ever predict this, because it depends on ENGINE-INTERNAL type coercion that can exist for any field, present or future; reimplementing every such coercion rule in `PromotionPipeline`/`config_contract.py` would be an unbounded, permanently-incomplete parallel model of the engine. There is no config-level fix that closes this class for good, because "will these two configs behave identically" is fundamentally a question about the engine's behavior, not the config's shape. **Fixed in the same PR by changing the dedupe strategy entirely, not by adding another config-classification layer** (operator instruction: "fix it for real instead of patching"): the walk-forward/backtest simulation is deterministic given the same effective inputs (C7 data_version pinning already guarantees byte-identical inputs across re-runs), so two variants that behave identically MUST produce bit-identical metrics regardless of WHY they behave identically. `PromotionPipeline.run()`'s `n_finite_sensitivity_variants` dedupe (`backtesting/validation/promotion_pipeline.py`) now dedupes on each row's OBSERVED outcome -- the tuple `(oos_sharpe, oos_max_drawdown, trade_count, avg_is_sharpe)` `ParameterSensitivityRow` already carries from the REAL simulation -- instead of trying to predict equivalence from the config. This is not a reimplementation of the engine's coercion rules; it reads the engine's own answer directly, so it cannot drift out of sync with the engine no matter what coercion/aliasing exists now or is added later, and it retroactively subsumes every prior round's fix (raw-param duplication, ancestor/descendant collapse, informational fields, logging-only fields, section-level overrides) under one mechanism. `is_behavioral`/`prune_to_behavioral_leaves` (BUG-093/094) remain in `config_contract.py`, still used by `_resolve_frozen_grid`'s pre-flight quality gate (now explicitly documented as fail-fast/efficiency only, no longer load-bearing for dedupe correctness). New test `test_coerced_values_collapse_to_one_effective_variant` pins Codex's exact `[1, 1.0, True]` example. Several existing round 5-9 tests were updated: `_sweep_result()`'s default fixture previously gave every row an identical hardcoded `oos_sharpe=0.9` regardless of index (harmless under the old config-based dedupe, but wrong now -- fixed to vary per row for genuinely-distinct-params cases, matching realistic per-variant simulation results); `test_ancestor_descendant_grid_paths_collapse_to_same_effective_config` and the renamed/repurposed `test_nested_dict_param_values_do_not_crash_the_dedupe` had rows meant to represent "same effective config" with mismatched metrics (a latent inconsistency invisible under config-based dedupe, now corrected to give same-outcome rows identical metrics, matching engine determinism). |
| BUG-097 | Backtesting/Promotion (Gate 04, C8 precondition) | P2 | F2 | Fixed (PR #50, 04-4W) | Found during the same round-10 fix pass as BUG-096 (self-identified while auditing `_resolve_frozen_grid` for other fail-loud-but-wasteful gaps after the operator's "fix it for real" instruction, not a separate Codex finding). A `param_grid` key/candidate the backtest config contract cannot accept at all -- a REJECTED section (`constraints`, `risk_model`), a genuinely unknown/typo'd field, an out-of-range `portfolio.method` value, an `execution.*` value mismatching the fixed `FillSimulator`, or a dot-path absent from the base config entirely -- previously surfaced only when `ParameterSweeper.sweep` actually tried that variant, which is AFTER the (expensive) walk-forward leg has already run and recorded trial evidence (Stage 1 runs before Stage 3). `UnsupportedStrategyConfigError`/`ExecutionConfigMismatchError` are deliberately not caught as a NaN variant (see their docstrings -- "must propagate and halt"), and `_apply_params`/`_set_nested`'s `KeyError` for a nonexistent dot-path is also uncaught by design, so either crashed the whole promotion run instead of failing the SAME fail-closed precondition `_resolve_frozen_grid` already enforces for every other malformed-grid shape (empty grid, non-list candidates, non-behavioral keys). **Fixed in the same PR**: `_resolve_frozen_grid` now validates every `(key, candidate)` pair by applying it to the base config and running the REAL `validate_backtest_config`/`assert_fill_simulator_matches_config` machinery (reused, not reimplemented -- avoiding a second, possibly-drifting copy of rejected/unknown/value-restricted logic), catching `KeyError` and `UnsupportedStrategyConfigError`, before any instrument runs. New tests `test_grid_with_rejected_section_key_fails_closed_before_walk_forward`, `test_grid_with_unknown_field_key_fails_closed_before_walk_forward`, and `test_grid_with_invalid_portfolio_method_value_fails_closed` pin each case. |
| BUG-098 | Backtesting/Promotion (Gate 04, C8 precondition) | P1 | F1 | Fixed (PR #50, 04-4W) | Found by Codex round-11 review on PR #50, a real edge case in BUG-096's fix landed the same round. Python's `NaN != NaN` under default equality, so two rows sharing the SAME (coerced) `oos_sharpe` but each independently producing `NaN` for an AUXILIARY metric -- most plausibly `avg_is_sharpe`, undefined when an in-sample fold has zero-variance returns -- did NOT collapse as dict keys in the round-10 outcome-tuple dedupe, even though they represent the identical simulated outcome; the finite-ness filter only checks `oos_sharpe`, not the other three fields. `{"portfolio.n_long": [1, 1.0, True]}` with `avg_is_sharpe=NaN` on every row could therefore still produce 3 dict entries for the same execution and clear `MIN_SENSITIVITY_SWEEP_VARIANTS`. **Fixed in the same PR**: added `_nan_safe(value)` (`backtesting/validation/promotion_pipeline.py`), which normalizes a NaN float to `None` (equal to itself, unlike NaN) before building the outcome-key tuple, so two independently-NaN auxiliary metrics correctly collapse. New tests `test_coerced_values_with_independently_nan_auxiliary_metric_still_collapse` (pins Codex's exact scenario) and `test_nan_safe_normalizes_nan_and_passes_through_other_values` (direct unit test of the helper). |
| BUG-099 | Backtesting/Promotion (Gate 04, C8 precondition) | P1 | F1 | Fixed (PR #50, 04-4W) | Found by Codex round-12 review on PR #50. The round-10 (BUG-096) outcome-based dedupe used four summary scalars (`oos_sharpe`/`oos_max_drawdown`/`trade_count`/`avg_is_sharpe`) as the identity key -- a NECESSARY but not SUFFICIENT condition for "two variants ran the identical simulation": two genuinely DIFFERENT observed return paths can coincidentally share all four aggregates (`trade_count` especially is low-cardinality), wrongly collapsing distinct variants. In the adversarial direction Codex flagged, several distinct LOSING variants with coincidentally-matching summary stats could collapse to one while a distinct WINNER stayed separate, skewing the recomputed `positive_fraction` upward and potentially flipping `curve_fit` to `robust`. **Fixed in the same PR by strengthening the identity signal, not by reverting to config-shape prediction** (which round-10 already proved insufficient): `ParameterSensitivityRow` gains `oos_returns_fingerprint: str` -- a SHA-256 hash of the FULL, order-sensitive OOS return sequence (`backtesting.validation.parameter_sensitivity.fingerprint_returns`), computed once per variant inside `ParameterSweeper.sweep()` from `wf.oos_returns` before it's discarded. `PromotionPipeline.run()`'s outcome key now folds this fingerprint in alongside the four scalars. This remains purely OUTPUT-based (reads what the engine actually produced; does not model or predict it), so it inherits round-10's "cannot drift out of sync with the engine" property while requiring the ENTIRE observed return sequence to match, not just four aggregates of it -- for two variants to collide now, their full day-by-day OOS return sequence must be byte-identical. Defaults to `""` for the (test-only) rows constructed without a real backtest, harmless since it's a constant suffix that doesn't change any existing test's equality/inequality outcome. New tests: `fingerprint_returns`-level unit tests (`test_fingerprint_returns_identical_series_match`, `_differing_series_differ`, `_order_sensitive`) in `test_parameter_sensitivity.py`; `test_coincidental_metric_match_does_not_collapse_distinct_executions` in `test_promotion_pipeline.py` pins Codex's exact adversarial scenario (2 rows with identical summary metrics but different fingerprints stay distinct; a true duplicate with matching fingerprint still collapses). |
| BUG-086 | Docs/Process | P2 | F2 | Open | Found by Codex round-2 review on PR #50. `docs/plans/04-strategy-selection-protocol-design.md` (around line 515) says the forthcoming `validated`-status migration (04-5) should be revision `018`, but PR #50 already consumed `018` for `018_promotion_decision_eval_window.py` (chained onto `017`) as part of the R1-A fix -- following the doc as written would create a duplicate revision `018` and break Alembic's linear upgrade path. Deliberately deferred rather than fixed in PR #50 (PM decision, 2026-08-08): it is a planning-doc pointer with no code or schema impact until 04-5 is actually built, and 04-5 hasn't started. **Required action before 04-5 starts:** update the doc to assign the `validated`-status migration revision `019` with `down_revision = "018"`, consistent with `018_promotion_decision_eval_window.py`'s own docstring. |
| BUG-087 | Backtesting/Registry (Gate 04, C8 precondition) | P2 | F2 | Fixed (PR #50, 04-4W) | Found by Codex round-3 review on PR #50. `EvaluationWindow.__post_init__` (`strategy_registry/evaluation_window.py`) only checked `start <= end`, with no check that `start`/`end` are actually `date` instances. Two failure modes both pass the comparison silently: (1) `datetime` subclasses `date`, so a caller passing two `datetime` values constructs a supposedly-validated window; (2) two ISO date strings compare lexicographically the same way two dates do, so a caller passing strings also passes. Either way the invalid window looks valid at construction, `TrialRecorder` persists/freezes a trial on the strength of it, and only later does it raise `TypeError` when compared against real `date` objects (e.g. in `DataHandler.trading_dates`) or fail DATE-column persistence -- potentially leaving an errored trial and a frozen hypothesis behind. **Fixed in the same PR**: `__post_init__` now rejects any value that is not a `date` instance or that IS a `datetime` instance (checked explicitly since `isinstance(dt, date)` is `True` for `datetime`), raising `TypeError` before the `start <= end` comparison runs. Five new tests in `tests/strategy_registry/test_evaluation_window.py` pin `datetime` start/end, ISO string start/end, and `None` all raising `TypeError`. Fixed directly rather than deferred (operator decision, 2026-08-08) since the class's whole purpose is guaranteeing "every consumer may assume a valid window" and the fix is small/local. |
