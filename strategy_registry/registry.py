...
class StrategyRegistry:
    ...
    def record_run(
        ...
        eval_start_date/eval_end_date (migration 016, PR #49 round-4
        completion sweep / Codex R4-B) are the EFFECTIVE evaluation window
        this run actually ran over. Required (both non-null, start <= end)
        for the same run_types that require data_version
        (backtest/walk_forward) -- since
        ...
        if run_type in _REQUIRE_DATA_VERSION:
            if eval_start_date is None or eval_end_date is None:
                raise ValueError(
                    ...
                    f"run_type='{run_type}' (PR #49 round-4 completion sweep, "
                    f"migration 016). Without them, two '{run_type}' runs "
                    f"recorded over different evaluation windows under the "
                    f"same (strategy_id, config_hash, data_version) are "
