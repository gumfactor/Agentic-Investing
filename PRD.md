...
| M5.8 | Streamlit dashboard (7 pages): Overview, Positions & P&L, Risk Monitor, Blotter Approval (F7.4 — C1 gate, replaces CLI confirmation), Performance (with multi-strategy comparison panel per F7.1), Signal & Strategy Explorer (F7.5), Audit Trail; two new backend tables (`portfolio_snapshots`, `strategy_simulations`); `monitor` and `report` skills operational | Engineering | 39 |
