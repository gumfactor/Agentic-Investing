...
_WINDOW = 21
_MIN_PERIODS = 21  # full window (BUG-010); see also require_full_window gate below
...
def compute_volume_up_down_ratio_21d_scores(
    ...
    # NaN daily_ret compares False to both `> 0` and `< 0`, so a missing
    # return silently contributes 0 volume to *both* sums instead of being
    # excluded — raising min_periods alone cannot catch this, since the
    # summed series itself is never NaN. Gate the final ratio explicitly on
    # the trailing window having a full set of valid returns (BUG-010).
    ...
    ratio = require_full_window(ratio, daily_ret, _WINDOW)
